"""
Optimized MobileNetV3 Model Training Script
...
# Setup logging
log_file = os.path.join(MODEL_RESULTS_PATH, "mobilenetv3_training.log")
logging.basicConfig(
    level=logging.INFO,
...
class Config:
    def __init__(self):
        ...
        # MobileNetV3 specific settings
        self.mobilenet_size = "large"  # Options: "large" or "small"
        
        # Hardware-specific optimizations - FIXED VALUES for RTX 3060 Laptop (6GB)
        self.use_amp = True  # Automatic Mixed Precision
        self.memory_efficient_attention = False  # Not applicable for MobileNetV3
        ...
        self.batch_size = 128  # Increased for MobileNetV3 (smaller model)
        self.gradient_accumulation_steps = 1  # No need for accumulation with larger batch size
        ...
        self.mobilenet_input_size = 224  # Required size for MobileNetV3 model
...
def get_dataloaders(config):
    ...
    # For MobileNetV3 models, we need to use ImageNet normalization
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    
    if config.use_zero_padding:
        # Calculate padding required to bring 32x32 to 224x224
        pad_size = (config.mobilenet_input_size - config.input_size) // 2
        ...
        logger.info(f"Using resize from {config.input_size}x{config.input_size} to "
                    f"{config.mobilenet_input_size}x{config.mobilenet_input_size}")
        
        # Transform for training with resize and data augmentation
        train_transform = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.Resize(config.mobilenet_input_size, antialias=True),
            transforms.ToTensor(),
            normalize,
        ])
...
# Model Definition
def create_mobilenetv3_model(config):
    """Create and initialize a MobileNetV3 model for CIFAR-10"""
    if config.mobilenet_size == "large":
        if config.pretrained:
            # Load pretrained MobileNetV3-Large model
            logger.info("Loading pretrained MobileNetV3-Large model...")
    ...
    logger.info(f"Model created: MobileNetV3-{config.mobilenet_size.capitalize()} adapted for CIFAR-10 with {num_classes} classes")
    return model
...
def train_model(config):
    ...
    # Create model
    model = create_mobilenetv3_model(config)
...
def plot_history(history, save_path=None):
    ...
    plt.suptitle(f"Training History - MobileNetV3 Model for CIFAR-10", fontsize=16)
...
if __name__ == "__main__":
    try:
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        script_destination = os.path.join(SCRIPTS_PATH, f"mobilenetv3_{timestamp}.py")
        ...
        try:
            ...
            if 'get_ipython' in globals():
                ...
                with open(script_destination, 'w') as f:
                    f.write(f"# MobileNetV3-{config.mobilenet_size.capitalize()} Training Script (Optimized for RTX 3060 Laptop)\n")
        ...
        # Train model - use new model creation function
