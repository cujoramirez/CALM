"""
Optimized InceptionV3 Teacher Model Training Script
...
# Setup logging
log_file = os.path.join(MODEL_RESULTS_PATH, "inception_v3_teacher_training.log")
logging.basicConfig(
    level=logging.INFO,
...
class Config:
    def __init__(self):
        # General settings
        self.seed = 42
        self.model_name = "inception_v3_teacher"
        ...
        self.gradient_accumulation_steps = 4  # Accumulate for effective batch of 128
        ...
        self.inception_input_size = 299  # Required size for InceptionV3 model
        self.num_workers = 0  # Reduced from 8 to prevent memory contention
...
def get_dataloaders(config):
    ...
    # For InceptionV3 models, we need to use ImageNet normalization
    ...
    if config.use_zero_padding:
        pad_size = (config.inception_input_size - config.input_size) // 2
        logger.info(f"Using zero padding with pad_size={pad_size} pixels on each side")
        train_transform = transforms.Compose(
            augmentation + [transforms.Pad(pad_size, fill=0)]
        )
        ...
        logger.info(f"Using resize from {config.input_size}x{config.input_size} to "
                    f"{config.inception_input_size}x{config.inception_input_size}")
        train_transform = transforms.Compose(
            augmentation + [transforms.Resize(config.inception_input_size, antialias=True)]
        )
        test_transform = transforms.Compose([
            transforms.Resize(config.inception_input_size, antialias=True),
            transforms.ToTensor(),
            normalize,
        ])
...
def create_inception_model(config):
    """Create and initialize an InceptionV3 model for CIFAR-10"""
    if config.pretrained:
        # Load pretrained InceptionV3 model
        logger.info("Loading pretrained InceptionV3 model...")
        weights = Inception_V3_Weights.IMAGENET1K_V1
        model = inception_v3(weights=weights, aux_logits=True)  # Enable aux_logits
    else:
        model = inception_v3(aux_logits=True)  # Enable aux_logits

    # Replace the classification head for CIFAR-10 with dropout
    ...
    # Replace the auxiliary classifier head for CIFAR-10 with dropout
    if model.aux_logits:
    ...
    logger.info(f"Model created: InceptionV3 adapted for CIFAR-10 with {num_classes} classes (aux_logits enabled)")
    return model
    ...
    for batch_idx, (inputs, targets) in enumerate(pbar):
        try:
            ...
            with autocast(device_type='cuda', enabled=config.use_amp):
                ...
                # If aux_logits is enabled, outputs is (main, aux)
                if model.aux_logits and isinstance(outputs, tuple):
                    main_output, aux_output = outputs
                    ce_loss = criterion(main_output, targets)
                    aux_loss = criterion(aux_output, targets)
                    total_ce_loss = ce_loss + 0.4 * aux_loss  # Add aux loss with weight 0.4
                    # Add calibration loss with current weight from curriculum scheduling
                    if current_cal_weight > 0:
                        cal_loss = CalibrationMetrics.calibration_loss(main_output, targets)
                        loss = config.ce_weight * total_ce_loss + current_cal_weight * cal_loss
                    else:
                        loss = total_ce_loss
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(val_loader, desc="Validating"):
            try:
                ...
                all_probs.append(F.softmax(outputs_for_acc, dim=1).cpu())
...
def plot_history(history, save_path=None):
    ...
    plt.suptitle(f"Training History - InceptionV3 Teacher Model for CIFAR-10", fontsize=16)
...
def visualize_examples(model, dataloader, device, config, num_examples=5):
    ...
    def denormalize(img):
        ...
        if img.shape[0] == 299:  # Check if we're using the 299x299 size
...
if __name__ == "__main__":
    try:
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        script_destination = os.path.join(SCRIPTS_PATH, f"inception_v3_teacher_{timestamp}.py")
        ...
        try:
            ...
            if 'get_ipython' in globals():
                ...
                with open(script_destination, 'w') as f:
                    f.write(f"# InceptionV3 Teacher Training Script (Optimized for RTX 3060 Laptop)\n")
