...
class EvalConfig:
    def __init__(self):
        ...
        self.checkpoint_path = r"C:\Users\Gading\Downloads\Research\Models\MobileNetV3\checkpoints\mobilenetv3_20250508_053015_best.pth"
...
def load_model(config, device):
    """Load the MobileNetV3 model from checkpoint"""
    ...
    try:
        ...
        model.to(device)
        model.eval()
        print("[INFO] MobileNetV3 model loaded successfully and set to evaluation mode")
...
def run_inference(model, loader, config, device):
    ...
    all_preds = []
    all_probs = []
    all_logits = []  # Add list to store logits
    ...
    with torch.no_grad():
        for images, targets in tqdm(loader, desc="Evaluation"):
            ...
            # Get predictions
            logits = outputs  # Store raw logits
            ...
            # Free memory
            del images, outputs, probs, preds, logits
            torch.cuda.empty_cache()
    ...
    return np.array(all_targets), np.array(all_preds), all_probs, all_logits  # Return logits


####################################
# 6. Evaluation Metrics
####################################
def analyze_results(y_true, y_pred, y_probs, y_logits, class_names, config):
    ...
    # 3. Generate classification report (includes precision, recall, F1-score per class)
    ...
    with open(f"{config.output_dir}/classification_report.txt", "w") as f:
        ...
        # Calculate and add F1 scores
        f1_micro = f1_score(y_true, y_pred, average='micro')
        f1_macro = f1_score(y_true, y_pred, average='macro')
        f1_weighted = f1_score(y_true, y_pred, average='weighted')
        
        print(f"\n[RESULT] F1 Score (Micro): {f1_micro:.3f}")
        print(f"[RESULT] F1 Score (Macro): {f1_macro:.3f}")
        print(f"[RESULT] F1 Score (Weighted): {f1_weighted:.3f}")
        
        f.write(f"\nF1 Score (Micro): {f1_micro:.3f}\n")
        f.write(f"F1 Score (Macro): {f1_macro:.3f}\n")
        f.write(f"F1 Score (Weighted): {f1_weighted:.3f}\n")

        # Calculate Cross-Entropy Loss
        # Ensure y_true is in the correct format (long tensor for CrossEntropyLoss)
        # y_logits should be float tensor
        loss_fn = CrossEntropyLoss()
        loss = loss_fn(torch.from_numpy(y_logits).float(), torch.from_numpy(y_true).long())
        print(f"[RESULT] Cross-Entropy Loss: {loss.item():.4f}")
...
def visualize_predictions(model, test_dataset, config, device, num_examples=5):
    ...
    fig.suptitle("CIFAR-10 Prediction Examples (MobileNetV3)", fontsize=14, y=0.98)
    ...
    plt.figtext(0.5, 0.01, 
               f"MobileNetV3-Large evaluation on CIFAR-10 test set", 
...
def visualize_gradcam(model, test_dataset, config, device):
    ...
    # Initialize GradCAM with the appropriate layer for MobileNetV3
    # For MobileNetV3, we use the last convolutional layer before classification
    ...
    fig.suptitle("GradCAM Visualizations for CIFAR-10 Classes (MobileNetV3)", fontsize=14, y=0.98)
...
def main():
    ...
    print("=" * 50)
    print("MobileNetV3 CIFAR-10 Evaluation Pipeline")
    print("=" * 50)
    ...
    try:
        ...
        # 3. Run inference
        targets, predictions, probabilities, logits = run_inference(model, test_loader, config, device)  # Capture logits
        
        # 4. Generate metrics
        accuracy = analyze_results(targets, predictions, probabilities, logits, config.classes, config)  # Pass logits and probabilities
