...
class EvalConfig:
    def __init__(self):
        ...
        self.checkpoint_path = r"C:\Users\Gading\Downloads\Research\Models\ResNet50\checkpoints\resnet50_teacher_20250508_022222_best.pth"
        ...
        # ImageNet normalization (used by ResNet50)
...
def load_model(config, device):
    """Load the ResNet50 model from checkpoint"""
    ...
    try:
        ...
        model.to(device)
        model.eval()
        print("[INFO] ResNet50 model loaded successfully and set to evaluation mode")
...
def run_inference(model, loader, config, device):
    ...
    return np.array(all_targets), np.array(all_preds), all_probs, np.array(all_logits)  # Return logits
...
def analyze_results(y_true, y_pred, y_logits, class_names, config):
    ...
    # Convert logits and true labels to tensors for loss calculation
    y_logits_tensor = torch.tensor(y_logits, dtype=torch.float32)
    y_true_tensor = torch.tensor(y_true, dtype=torch.long)
    loss = F.cross_entropy(y_logits_tensor, y_true_tensor).item()
    print(f"[RESULT] Test Cross-Entropy Loss: {loss:.4f}")
    ...
    # 4. Extract and print key metrics (Macro and Weighted Averages)
    ...
    print(f"\n[RESULT] Macro F1-Score: {macro_f1:.3f}")
    print(f"[RESULT] Weighted F1-Score: {weighted_f1:.3f}")
    print(f"[RESULT] Macro Precision: {macro_precision:.3f}")
    print(f"[RESULT] Weighted Precision: {weighted_precision:.3f}")
    print(f"[RESULT] Macro Recall: {macro_recall:.3f}")
    print(f"[RESULT] Weighted Recall: {weighted_recall:.3f}")
    
    # 5. Save report to file
    with open(f"{config.output_dir}/classification_report.txt", "w") as f:
        f.write(f"Test Accuracy: {accuracy:.2f}%\n")
        f.write(f"Test Cross-Entropy Loss: {loss:.4f}\n\n")
...
def visualize_predictions(model, test_dataset, config, device, num_examples=5):
    ...
    plt.figtext(0.5, 0.01, 
               f"ResNet50 evaluation on CIFAR-10 test set", 
...
def visualize_gradcam(model, test_dataset, config, device):
    ...
    # Initialize GradCAM with the appropriate layer for ResNet50
    target_layer = model.layer4[-1]  # Last layer in ResNet50
    ...
    fig.suptitle("GradCAM Visualizations for CIFAR-10 Classes (ResNet50)", fontsize=14, y=0.98)
...
def main():
    ...
    print("=" * 50)
    print("ResNet50 CIFAR-10 Evaluation Pipeline")
    print("=" * 50)
